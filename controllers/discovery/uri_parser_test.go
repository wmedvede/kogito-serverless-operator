...
func TestParseKnativeServicesURI2(t *testing.T) {

	knService1 := "knative:services.v1.serving.knative.dev/usecase1/kn-service1"
	knService2 := "knative:usecase1/kn-service2"

	if result1, err := ParseUri(knService1); err != nil {
		fmt.Println("error! ")
	} else {
		fmt.Println(result1.String())
	}

	if result2, err := ParseUri(knService2); err != nil {
		fmt.Println("error! ")
	} else {
