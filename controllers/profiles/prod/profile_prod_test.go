...
func Test_Reconciler_ProdOps(t *testing.T) {
	...
	result, err := NewProfileForOpsReconciler(client, &rest.Config{}).Reconcile(context.TODO(), workflow)
...
func Test_Reconciler_ProdCustomPod(t *testing.T) {
	...
	_, err := NewProfileReconciler(client, &rest.Config{}).Reconcile(context.TODO(), workflow)
	assert.NoError(t, err)
...
func Test_reconcilerProdBuildConditions(t *testing.T) {
	...
	result, err := NewProfileReconciler(client, &rest.Config{}).Reconcile(context.TODO(), workflow)
	assert.NoError(t, err)
